...
export default function Tastes() {
	...
	const [selectedQuantity, setSelectedQuantity] = React.useState(() => [...Array(tempItems.length).keys()].map(i => i = 1));
	const [slideIndex, setSlideIndex] = React.useState(() => 0);

	// const minIndex = 0
	const maxIndex = tempItems.length - 3 //(5) -> 2
			...
			<div className="tastes__wrapper">
				<div className="tastes__text">
					...
					<div className="tastes__text__description">
						<p>
							Из нашей коллекции <span>Лёд Влёд</span> каждый сможет выбрать для себя мороженое по вкусу
						</p>
					</div>
				</div>
					...
					<div className="tastes__slider__content" id='slider'> {/*  !!!!!!!  */}
						{
							tempItems.map((it, index) => (
								<div className="tastes__slider__slide" key={index} id={`slide_${index}`}>
									...
									<div className="tastes__slider__slide__counter">
										<div className="tastes__slider__slide__counter__plate">
											<div className="tastes__slider__slide__counter__plate__button">
